...
// ── POST /api/wishes/:id/react — 给某条 wish 增加 1 个 reaction ──
// body: { type: 'pray' | 'heart' | 'sparkle' | 'thought' }
app.post('/api/wishes/:id/react', async (req, res) => {
  const wishId = req.params.id;
  const reactionType = req.body && req.body.type;
  const VALID = ['pray', 'heart', 'sparkle', 'thought'];
  if (!VALID.includes(reactionType)) {
    return res.status(400).json({ error: 'invalid reaction type' });
  }
  try {
    // 先读当前这条 wish 的 reactions
    const getR = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?id=eq.${encodeURIComponent(wishId)}&select=reactions`, {
      headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
    });
    if (!getR.ok) {
      const t = await getR.text();
      return res.status(getR.status).send(t);
    }
    const rows = await getR.json();
    if (!rows.length) return res.status(404).json({ error: 'wish not found' });

    const current = rows[0].reactions || { pray: 0, heart: 0, sparkle: 0, thought: 0 };
    const next = {
      pray: current.pray || 0,
      heart: current.heart || 0,
      sparkle: current.sparkle || 0,
      thought: current.thought || 0,
    };
    next[reactionType] = (next[reactionType] || 0) + 1;

    const patchR = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?id=eq.${encodeURIComponent(wishId)}`, {
      method: 'PATCH',
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
    ...
    if (!patchR.ok) {
      ...
      console.error('[POST react] PATCH error:', patchR.status, t);
      return res.status(patchR.status).send(t);
    }
    const updated = await patchR.json();
    res.json({ reactions: next, wish: updated[0] });
  } catch (err) {
    console.error('[POST /api/wishes/:id/react]', err.message);
    res.status(500).json({ error: err.message });
  }
});
